...
<style scoped>
  .active{
    background-color: rgba(24, 138, 156, 0.44);
  }
</style>
...
export default {
  data() {
    return {
      ...
      routeCurrent: this.$router.currentRoute
    }
  },
  watch: {
    '$route': 'setCurentRoute'
  },
  computed: {
    // user: function () {
  ...
  methods: {
    setCurentRoute() {
      this.routeCurrent = this.$router.currentRoute
    }
  },
  mounted() {
    ...
    EventBus.$on('user-data', user => {
      this.user = JSON.parse(user)
      console.log(`Oh, that's nice. It's gotten ${user.name} clicks! :)`)
    });
